...
/** Generate a mailto: link for batch material orders (multiple projects/parts). */
export function generateMaterialOrderMailto(
  parts: MaterialPart[],
  supplierEmail = ""
): string {
  const families = [...new Set(parts.map((p) => p.familyName))];
  const subjectFamily = families.length === 1 ? families[0] : "Multiple Projects";
  const subject = `Material Request — ${subjectFamily} — ${today()}`;

  // Group by family
  const grouped: Record<string, MaterialPart[]> = {};
  parts.forEach((p) => {
    (grouped[p.familyName] ??= []).push(p);
  });

  const bodyBlocks = Object.entries(grouped).map(([family, ps]) => {
    const lines = ps
      .map((p) => {
...
export function generateExternalOpMailto(
  ...
  const { partId, familyName, operationNotes, supplierEmail = "" } = data;
